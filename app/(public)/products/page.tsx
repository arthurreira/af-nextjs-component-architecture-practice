...
function PlanCard({ plan }: { plan: Product }) {
  ...
  const cardContent = (
    <>
      ...
      <div className="relative overflow-hidden rounded-lg border bg-background px-6 pt-5 pb-4">
        ...
        <Icon weight="duotone" className="mb-5 text-primary relative z-10" />
        <h3 className="font-medium text-2xl tracking-tight relative z-10">{plan.name}</h3>
        <p className="mt-1 mb-2 text-muted-foreground relative z-10">{plan.description}</p>
      </div>
  ...
  return (
    <div className={cn("relative flex flex-col rounded-xl border bg-muted/50 p-1 dark:bg-muted/75")}>
      {cardContent}
    </div>
  )
}